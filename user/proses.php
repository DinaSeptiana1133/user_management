...
if(isset($_POST['ganti'])){
    ...
    if ($sql->num_rows == 0){
        ...
        echo "<script>alert('password tidak sama'); window.location='$redirect';</script>";
    } elseif ($pasnew !==  $conpas){
        echo "<script>alert('password tidak cocok'); window.location='$redirect';</script>";
    } else {
        $query="update users set password='".password_hash($pasnew, PASSWORD_DEFAULT)."' where id='".$cek['id']."'";
        $sql = $db->query($query);
        if($sql){
            echo "<script>alert('Password berhasil diubah'); window.location='$redirect';</script>";
        } else {
            echo "<script>alert('Password gagal diubah'); window.location='$redirect';</script>";
        }
    }
}
?>